...
try {
  ...
  if ($jakieposty = mysqli_query($baza, "SELECT * FROM `powiadomienia` where `powiadomienia`.`id_odbiorcy` = '$sesja' AND `powiadomienia`.`typ` = 1 AND `powiadomienia`.`wyswietlono` = 0 order by `id` DESC LIMIT 1")) {
    ...
    while ($ktoreposty = mysqli_fetch_assoc($jakieposty)) {
      ...
      $idrekordu = (int)$ktoreposty['id'];
      if(!mysqli_query($baza,"UPDATE `powiadomienia` SET wyswietlono = 1 WHERE `id` = '$idrekordu'")){
       throw new Exception("Nie udało się wyświetlić");
     }
