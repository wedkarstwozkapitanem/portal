...
try {
  ...
  global $baza;
    ...
    $zapytanie_post = mysqli_query($baza,"SELECT `posty`.`idp`,`posty`.`iduzytkownika`,`posty`.`tresc`,`posty`.`foty`,`posty`.`datadodania`,`posty`.`publiczny`,`uzytkownicy`.`id`,`uzytkownicy`.`imie`,`uzytkownicy`.`nazwisko`,`uzytkownicy`.`profilowe`,`uzytkownicy`.`folder` FROM `posty` LEFT JOIN `uzytkownicy` ON `posty`.`iduzytkownika` = `uzytkownicy`.`id` where  `posty`.`usunieto` = 0 order by `posty`.`idp` desc");
  (int) $i = (int)0;
   ...
   // $dane_posta[$i] += mysqli_fetch_assoc(mysqli_query($baza, "SELECT `id`,`imie`,`nazwisko`,`profilowe`,`folder` FROM `uzytkownicy` where `id` = '$id_uzytkownika'"));
    ...
    $dane_posta[$i]['czymoj'] = mysqli_num_rows(mysqli_query($baza,"SELECT * FROM `posty` WHERE `iduzytkownika` = '$sesja' AND `idp`='$id_posta' AND `usunieto` = 0")) === 0 ? false : true;
    $i++;
    }
